...
Attributes::Attributes(void) { reset(); }
...
bool Attributes::isBold(void) const { return cBold; }

bool Attributes::isUnderline(void) const { return cUnderline; }

bool Attributes::isItalic(void) const { return cItalic; }

void Attributes::setBold(const bool v) { cBold = v; }

void Attributes::setItalic(const bool v) { cItalic = v; }

void Attributes::setUnderline(const bool v) { cUnderline = v; }
...
Console::Console(GrafPtr port, Rect r) { Init(port, r); }
...
void Console::Init(GrafPtr port, Rect r) {
  ...
  chars = std::vector<AttributedChar>(rows * cols,
                                      AttributedChar(' ', currentAttr));
...
void Console::SetAttributes(Attributes aa) {
  TextFace(aa.isBold()            ? bold + condense
           : 0 + aa.isUnderline() ? underline
           : 0 + aa.isItalic()    ? italic
                                  : 0);
}

Rect Console::CellRect(short x, short y) {
  return {(short)(bounds.top + y * cellSizeY),
          (short)(bounds.left + x * cellSizeX),
          (short)(bounds.top + (y + 1) * cellSizeY),
          (short)(bounds.left + (x + 1) * cellSizeX)};
}
...
void Console::DrawCells(short x1, short x2, short y, bool erase) {
  Rect r = {(short)(bounds.top + y * cellSizeY),
            (short)(bounds.left + x1 * cellSizeX),
            (short)(bounds.top + (y + 1) * cellSizeY),
            (short)(bounds.left + x2 * cellSizeX)};
  if (cursorDrawn) {
...
void Console::Draw(Rect r) {
  ...
  short maxRow =
      std::min((int)rows, (r.bottom - bounds.top + cellSizeY - 1) / cellSizeY);

  short minCol = std::max(0, (r.left - bounds.left) / cellSizeX);
  short maxCol =
      std::min((int)cols, (r.right - bounds.left + cellSizeX - 1) / cellSizeX);
...
bool Console::ProcessEscSequence(char c) {
  ...
  case State::waitingForM:
    if (c == 'm')
      ...
      sequenceState =
          State::noSequence; // Unrecognized sequence (but we end it anyway!)
    break;
...
void Console::Reshape(Rect newBounds) {
  ...
  std::vector<AttributedChar> newChars(newRows * newCols,
                                       AttributedChar(' ', currentAttr));
...
char Console::WaitNextChar() { return 0; }
#endif
